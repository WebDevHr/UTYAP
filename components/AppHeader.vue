      ...
      <nav class="mx-auto flex max-w-7xl items-center justify-between py-4" aria-label="Global" v-if="!mobileMenuOpen" >
        ...
        <PopoverGroup class="hidden lg:flex lg:gap-x-6">
            ...
            <Popover v-for="(navItem, index) in navItems" :key="index"  class="relative" v-slot="{ open }">
              ...
              <transition v-if="navItem.subItems.length != 0" enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 translate-y-3" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition ease-in duration-150" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-1">
                <PopoverPanel v-slot="{ close }" class="absolute -left-8 top-full z-10 mt-4 w-screen max-w-2xl overflow-hidden rounded-3xl bg-white shadow-lg ring-1 ring-gray-900/5">
                  <div class="p-4">
                    <div 
                      v-for="item in navItem.subItems" 
                      :key="item.name" 
